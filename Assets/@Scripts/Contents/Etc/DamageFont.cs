...
namespace STELLAREST_F1
{
    ...
    //                     EFontAnimationType fontOutAnimType = EFontAnimationType.GoingUp)
    //     {
    //         if (isCritical)
    //         {
    //             //DamageFont criticalFont = Managers.Object.ShowCriticalFont(position);
    //             // criticalFont.DoOutAnimation(EFontAnimationType.OutGoingUp);
    //         }

    //         // --- TEST CHANGE FONT TYPE
    //         _damageText.font = Managers.MonoContents.GetFontAsset(EFontType.Comic);
    //         // --- TEST CHANGE FONT TYPE
    ...
    //         Managers.MonoContents.PlayInGameFontAnimation(this.gameObject, _rectTransform, fontOutAnimType);
    //     }
    ...
    //     /* PREV 
    //     private void DoOutAnimation(EFontAnimationType animType)
    //     {
    //         switch (animType)
    //         {
    //             case EFontAnimationType.OutGoingUp:
    //                 PlayOutGoingUp();
    //                 break;

    //             case EFontAnimationType.OutSmaller:
    //                 PlayOutSmaller();
    //                 break;

    //             case EFontAnimationType.OutFalling:
    //                 PlayOutFalling();
    //                 break;

    //             case EFontAnimationType.OutBouncingLeftUp:
    //                 PlayOutBouncingLeftUp();
    //                 break;

    //             case EFontAnimationType.OutBouncingRightUp:
    ...
    //     // --- BouncingUp Test
    //     private void PlayOutBouncingLeftUp()
    //     {
    ...
    //         seq.Append(_rectTransform.DOScale(1.5f, randomUpDuration).SetEase(Ease.OutCirc)) // scale 증가 (0.1 -> 1.5)
    //            .Join(_rectTransform.DOPath(pathPoints, randomUpDuration + randomDownDuration, PathType.CatmullRom).SetEase(Ease.OutCirc)); // 곡선을 따라 이동

    //         // 두 번째 단계: 자연스럽게 아래로 떨어지면서 크기 감소 및 페이드아웃
    //         seq.Append(_rectTransform.DOScale(0f, randomDownDuration).SetEase(Ease.InCirc)) // 빠르게 크기 감소 (1.5 -> 0)
